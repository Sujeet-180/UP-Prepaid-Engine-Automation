...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/PE_208.log'),
...
def generate_excel_report(account_data):
    ...
    excel_file = os.path.join(result_folder, "Report_PE_208.xlsx")
    ...
    try:
        ...
        test_case_id = "PE_208"
        test_case_description = "To verify prepaid ledger calculation for ST = 76U With Consumption = up to 400 kWh and MD = upto 150%"
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("PE_208 Test Plan - Starting Complete Process")
    logger.info("=" * 80)
    ...
    # Summary
    logger.info("=" * 80)
    logger.info("PE_208 Test Plan - All Steps Completed Successfully!")
    logger.info("=" * 80)
    logger.info(f"Account ID: {account_data['accountId']}")
    logger.info(f"Meter Serial No: {account_data['meterSrno']}")
    logger.info(f"Meter Install Date: {account_data['meterInstalldate']}")
    logger.info(f"Excel Report: Result_File/Report_PE_208.xlsx")
    logger.info("=" * 80)
