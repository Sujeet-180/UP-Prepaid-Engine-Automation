...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/PE_216.log'),
...
def generate_excel_report(account_data):
    ...
    excel_file = os.path.join(result_folder, "Report_PE_216.xlsx")
    ...
    try:
        ...
        test_case_id = "PE_216"
        test_case_description = "To verify prepaid ledger calculation for ST = 93 (Weekly minimum charge + Temporary Consumer)"
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("PE_216 Test Plan - Starting Complete Process")
    logger.info("=" * 80)
    ...
    # Summary
    logger.info("=" * 80)
    logger.info("PE_216 Test Plan - All Steps Completed Successfully!")
    logger.info("=" * 80)
    logger.info(f"Account ID: {account_data['accountId']}")
    logger.info(f"Meter Serial No: {account_data['meterSrno']}")
    logger.info(f"Meter Install Date: {account_data['meterInstalldate']}")
    logger.info(f"Excel Report: Result_File/Report_PE_216.xlsx")
    logger.info("=" * 80)
