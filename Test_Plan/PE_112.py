...
SANCTIONED_LOAD = 1
LOAD_UNIT = "KW"
METER_INSTALL_DATE = "2025-11-12T00:00:00"
NET_METER_FLAG = "N"
GREEN_ENERGY_FLAG = "N"
POWER_LOOM_COUNT = 0
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/PE_112.log'),
...
def fill_profile_instant_data(account_data):
    ...
    try:
        ...
        # Starting MD_W value (some initial value that progresses to 750)
        start_md_w = 120
        start_md_va = start_md_w / 0.90
        
        logger.info(f"Starting data generation with MD_W progressing from {start_md_w} to {MD_W}, MD_VA: {start_md_va:.2f} to {md_va:.2f}")
        ...
        for day in range(total_days):
            ...
            if total_days > 1:
                ...
                progress_ratio = 1
            # Progress from start_md_w to MD_W (750) by month_end + 1 day
            current_md_w = int(start_md_w + (MD_W - start_md_w) * progress_ratio)
            current_md_va = int(start_md_va + (md_va - start_md_va) * progress_ratio)
            
            # Ensure final value at month_end + 1 day is exactly 750
            if day == total_days - 1:
                current_md_w = MD_W
                current_md_va = md_va
...
def generate_excel_report(account_data):
    ...
    excel_file = os.path.join(result_folder, "Report_PE_112.xlsx")
    ...
    try:
        ...
        test_case_id = "PE_112"
        test_case_description = "To verify prepaid ledger calculation for ST = 10 (normal 10A with meter installation in mid of the month)"
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("PE_112 Test Plan - Starting Complete Process")
    logger.info("=" * 80)
    ...
    # Summary
    logger.info("=" * 80)
    logger.info("PE_112 Test Plan - All Steps Completed Successfully!")
    logger.info("=" * 80)
    logger.info(f"Account ID: {account_data['accountId']}")
    logger.info(f"Meter Serial No: {account_data['meterSrno']}")
    logger.info(f"Meter Install Date: {account_data['meterInstalldate']}")
    logger.info(f"Excel Report: Result_File/Report_PE_112.xlsx")
    logger.info("=" * 80)
