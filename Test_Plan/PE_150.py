...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/PE_150.log'),
...
def generate_excel_report(account_data):
    ...
    excel_file = os.path.join(result_folder, "Report_PE_150.xlsx")
    ...
    try:
        ...
        test_case_id = "PE_150"
        test_case_description = "To verify prepaid ledger calculation for ST = 20 (Minimum Monthly Charge - Season 2 where Sanction load Above 4kW)"
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("PE_150 Test Plan - Starting Complete Process")
    logger.info("=" * 80)
    ...
    # Summary
    logger.info("=" * 80)
    logger.info("PE_150 Test Plan - All Steps Completed Successfully!")
    logger.info("=" * 80)
    logger.info(f"Account ID: {account_data['accountId']}")
    logger.info(f"Meter Serial No: {account_data['meterSrno']}")
    logger.info(f"Meter Install Date: {account_data['meterInstalldate']}")
    logger.info(f"Excel Report: Result_File/Report_PE_150.xlsx")
    logger.info("=" * 80)
