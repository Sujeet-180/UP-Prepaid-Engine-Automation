...
SANCTIONED_LOAD = 8
LOAD_UNIT = "KVA"
METER_INSTALL_DATE = "2025-06-01T00:00:00"
NET_METER_FLAG = "N"
GREEN_ENERGY_FLAG = "N"
POWER_LOOM_COUNT = 0
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/PE_197.log'),
...
def generate_excel_report(account_data):
    ...
    excel_file = os.path.join(result_folder, "Report_PE_197.xlsx")
    ...
    try:
        ...
        test_case_id = "PE_197"
        test_case_description = "To verify prepaid ledger calculation for ST = 62T (ToU - season 1)"
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("PE_197 Test Plan - Starting Complete Process")
    logger.info("=" * 80)
    ...
    # Summary
    logger.info("=" * 80)
    logger.info("PE_197 Test Plan - All Steps Completed Successfully!")
    logger.info("=" * 80)
    logger.info(f"Account ID: {account_data['accountId']}")
    logger.info(f"Meter Serial No: {account_data['meterSrno']}")
    logger.info(f"Meter Install Date: {account_data['meterInstalldate']}")
    logger.info(f"Excel Report: Result_File/Report_PE_197.xlsx")
    logger.info("=" * 80)
